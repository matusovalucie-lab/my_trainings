...
function toggleSet(exerciseId, setId) {
  const session = state.activeSession; const ex = session.exercises.find(e => e.id === exerciseId); const set = ex.sets.find(s => s.id === setId);
  set.completed = !set.completed;
  if (ex.sets.every(s => s.completed)) {
    ex.status = "completed";
    const next = session.exercises.findIndex(e => e.status === "waiting");
    if (next >= 0) { session.currentIndex = next; session.exercises[next].status = "active"; }
    else { saveState(); navigate("finish"); return; }
  }
  saveState(); render();
}
function addSet() {
...
function postponeCurrent() {
  const session = state.activeSession; session.exercises[session.currentIndex].status = "postponed";
  let next = session.exercises.findIndex(e => e.status === "waiting");
  if (next < 0) next = session.exercises.findIndex(e => e.status === "postponed");
  if (next >= 0) { session.currentIndex = next; session.exercises[next].status = "active"; saveState(); render(); }
  else navigate("finish");
}
function finishWorkout() {
  const session = state.activeSession; session.completedAt = new Date().toISOString(); state.history.unshift(session); state.activeSession = null; saveState(); navigate("history");
}
...
function homeScreen() {
  ...
  <main class="workout-grid">${WORKOUTS.map(w => `<button class="workout-card ${w.theme}" data-workout="${w.id}"><span class="workout-dot"></span><span><strong>${w.name}</strong><small>${w.subtitle}</small></span><b>›</b></button>`).join("")}</main>
...
function workoutScreen() {
  const s = state.activeSession; if (!s) return homeScreen(); const ex = currentExercise(), meta = areaMeta(ex.area);
  return `${header(s.workoutName, null, `${locationLabel(s.location)} • ${s.currentIndex + 1}/${s.exercises.length}`)}<section class="training-card" style="--area:${meta.color}"><div class="exercise-area">${meta.icon} ${meta.name}</div><h2>${ex.cs}</h2><p class="english">(${ex.en})</p><p class="target">Cíl: ${ex.target}</p><div class="sets-header"><span>Série</span><span>kg</span><span>opak.</span><span></span></div><div class="sets">${ex.sets.map(set => `<div class="set-row"><strong>${set.number}</strong><input type="number" step="0.5" inputmode="decimal" placeholder="0" value="${set.weight}" data-set="${ex.id}:${set.id}:weight"><input type="number" inputmode="numeric" placeholder="0" value="${set.reps}" data-set="${ex.id}:${set.id}:reps"><button class="check ${set.completed ? "done" : ""}" data-complete="${ex.id}:${set.id}">${set.completed ? "✓" : ""}</button></div>`).join("")}</div><button class="add-set" data-action="add-set">+ Přidat sérii</button></section><div class="training-actions"><button data-action="postpone">Přeskočit prozatím</button><button data-action="close">Zavřít a uložit</button></div>`;
}
function finishScreen() {
  const s = state.activeSession, done = s.exercises.filter(e => e.status === "completed").length;
  return `${header("Trénink dokončen", "workout")}<section class="finish-card"><span class="finish-icon">✓</span><h2>${s.workoutName}</h2><p>${done} z ${s.exercises.length} cviků dokončeno</p><button class="primary ${s.theme}" data-action="finish">Uložit trénink</button></section>`;
}
