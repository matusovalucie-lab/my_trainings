...
let SETTINGS = { language: "cs", defaultLocation: "gym", weightUnit: "kg", version: "0.6.0-alpha.3" };
...
function workoutScreen() {
  ...
  const nextMeta = next ? areaMeta(next.area) : null;
  ...
  <section class="next-exercise-card" ${nextMeta ? `style="--area:${nextMeta.color}"` : ""}>
    <span>Další cvik</span>
    ${next
      ? `<strong>${next.cs}</strong><small>(${next.en})</small>`
      : `<strong>Poslední cvik v pořadí</strong>`}
  </section>
